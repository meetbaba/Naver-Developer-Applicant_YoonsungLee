/** @file         Rte_Lib.c
  ...
  *               RTE_ARPATCH                  = 3
  *
  * Template:     Rte_Lib_c.rb 33076 2013-02-14 17:05:09Z sha1yok
  */
...
#if (RTE_VFB_TRACE != 0)
      Rte_Task_Activate( RTE_TASK_FROM_REF( c->task ) );
...
Rte_Start(void)                          /* [$Satisfies $SWS 2569] */
{
  ...
  Rte_StartHook_Start();                               /* [$Satisfies $SWS 1238] */
   ...
   Rte_StartHook_Return();                              /* [$Satisfies $SWS 1239] */
...
Rte_Stop(void)                           /* [$Satisfies $SWS 2570] */
{
   ...
   Rte_StopHook_Start(); /* [$Satisfies $SWS 1238] */
   ...
   Rte_StopHook_Return();                               /* [$Satisfies $SWS 1239] */
...
#if defined(RTE_VFB_TRACE)
#if (RTE_VFB_TRACE != 0)
            Rte_Task_Activate( RTE_TASK_FROM_REF( ((Rte_QRefTaskType)q)->task ) );
#endif /* (RTE_VFB_TRACE != 0) */
#endif /* defined(RTE_VFB_TRACE) */
            if ( E_OK != ActivateTask( RTE_TASK_FROM_REF( ((Rte_QRefTaskType)q)->task ) ) ) /* [$Satisfies $SWS 2203] */
            {
               /* restore 'in' and 'used' */
               dynamic->in = old_in;
               dynamic->used--;
...
Rte_memcpy(P2VAR(void, AUTOMATIC, RTE_DATA) dst,
           ...
           uint16 length)
{
   P2VAR(uint8, AUTOMATIC, RTE_DATA)           d = dst;
   P2CONST(uint8, AUTOMATIC, RTE_APPL_DATA)    s = src;
