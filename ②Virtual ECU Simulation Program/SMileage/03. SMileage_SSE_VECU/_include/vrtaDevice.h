...
protected:
  ...
  void EventWithUVal(vrtaEvent *event, vrtaEventID evID, vrtaUInt uval) {
    event->devID = GetID();
    event->devEvent = evID;
    event->devEventLen = sizeof (event->devEmbeddedData.uVal);
  ...
  void EventWithText(vrtaEvent *event, vrtaEventID evID, const char *text) {
    event->devID = GetID();
    event->devEvent = evID;
    event->devEventLen = strlen(text) + 1;
