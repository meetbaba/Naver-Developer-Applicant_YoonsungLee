/** @file     GyroPlausi_MemMap.h
  *
  * @brief    RTE Sample SWC MemMap file
  *
  * @note     Generated by ETAS GmbH  RTA-RTE v5.4.1  (R4.0 backend version: v7.1.31 (Build 33134))
  *           at Wed Dec 17 18:21:49 2014
  */

#if defined (START_WITH_IF)
#elif defined(GyroPlausi_START_SEC_CODE)
#undef GyroPlausi_START_SEC_CODE
#define RTE_START_SEC_APPL_CODE
#elif defined(GyroPlausi_STOP_SEC_CODE)
#undef GyroPlausi_STOP_SEC_CODE
#define RTE_STOP_SEC_APPL_CODE

#endif /* START_WITH_IF */

#include "MemMap.h"

